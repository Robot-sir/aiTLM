...
export function isClassroomCategory(value) {
    return typeof value === "string" && value in CURRICULUM;
}
export function isClassroomItem(category, item) {
    return (isClassroomCategory(category) &&
        typeof item === "string" &&
        item in CURRICULUM[category]);
}
export function isValidLearningItem(category, item) {
    return isClassroomCategory(category) && isClassroomItem(category, item);
}
/** Look up the grounded spoken fact for an item, so the teacher never invents facts. */
export function getItemFact(category, item) {
    if (!isClassroomItem(category, item))
        return null;
    const entry = CURRICULUM[category][item];
